...
const AllToolsNMaterialsFinalData = ({route}) => {
    ...
    const getToolsNMaterialsDetails = async () => {
      try {
        ...
        if (materialsListInfo !== null && materialsListInfo !== undefined) {
            const mergedArray = materialsListInfo.reduce((acc, current) => {
            ...
            setPagesData(mergedArray);
            setPId(mergedArray.length+1);
            setPageHieght((mergedArray.length-1)*80);
        } 
        ...
        if (toolsListInfo !== null && toolsListInfo !== undefined) {
            const mergedToolArray = toolsListInfo.reduce((acc, current) => {
                // Check if an object with the same id exists in the accumulator array
           ...
           setPagesToolData(mergedToolArray);
        }
    } catch (error) {
...
export default AllToolsNMaterialsFinalData;
