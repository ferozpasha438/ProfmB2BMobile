...
const ListItem = ({ item,surveyPopup,deletePopup,foreclosePopup,addNotesPopup }) => {
  ...
  const surveyPopupS=(ticketNumber:string)=>{
    surveyPopup(ticketNumber);
  };
  const foreclosePopupS=(ticketNumber:any)=>{
    foreclosePopup(ticketNumber);
  };
  const addNotesPopupS=(ticketNumber:any)=>{
    addNotesPopup(ticketNumber);
  };
        ...
        <View style={styles.groupChildLayout1}>
          <View style={[styles.groupView, styles.groupParentLayout]}>
            ...
            <View style={[styles.frameParent5, styles.frameParentPosition3]}>
              ...
              <Pressable style={styles.mediumLayout} onPress={() =>surveyPopupS(item.ticketNumber)}>
                  <Text numberOfLines={1} style={[styles.survey, styles.approveTypo]}>Materials Arrived</Text>
              </Pressable>
            ...
            {modalVisible && (<View style={styles.modalContainer}>
                        ...
                        <Pressable
                          style={styles.listItemStyleLink}
                          onPress={() => addNotesPopupS(item.ticketNumber)}>
                            <Image
                                style={styles.groupIconPop}
                                resizeMode="cover"
...
const InTransit = () => {
 ...
 useEffect(() => {
    if(requestList.length<2)
    {
      ...
      setPageHeight(260+(requestList.length*280));
    }
}, [requestList]);
  ...
  const surveyPopup = async (ticketNumber:string) =>{
    setDeleteReason("");
    setDeleteTicketNumber(ticketNumber);
    setShowSurveyPopup(true);
 };
 const serveyTicket = async () => {
  WebServer.postDataWithToken('FomMobJobTicketHead/markAsWorkInProgress',token,deleteTicketNumber, serveyTicketCallback); 
 };
 const serveyTicketCallback = serveyReponse => {
  if(serveyReponse && serveyReponse.message!=undefined){
    if(serveyReponse.message==="Successful."){
      ...
      // setPageHeight(windowHeight);
       ...
       setShowSurveyPopup(false);
       Alert.alert("Materials arrived successfully.");
  ...
  const addNotesPopup = async (ticketNumber:any) =>{
    setDeleteReason("");
    setDeleteTicketNumber(ticketNumber);
  ...
  const addNotesToTicketCallback = reponse => {
    if(reponse && reponse.message!=undefined){
      if(reponse.message==="Successful."){
        ...
        Alert.alert("Notes added successfully.");
      }else{
        Alert.alert(reponse.message);
      }
    }
  ...
  const renderData = () => {
    return requestList.map((item:any) => (
      <ListItem key={item.id} item={item} surveyPopup={surveyPopup} deletePopup={deletePopup} foreclosePopup={foreclosePopup} addNotesPopup={addNotesPopup} />
...
const getRequestList = async (jwttoken:string) => {
    ...
    WebServer.postDataWithToken('FomMobJobTicketHead/getInTransitTicketsListPagination',jwttoken,params, requestListCallback);
  };
