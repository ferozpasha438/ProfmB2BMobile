...
const ListItem = ({ item,surveyPopup,deletePopup,foreclosePopup }) => {
  ...
  const surveyPopupS=(ticketNumber:string,isQuotationSubmitted:boolean,isPoRecieved:boolean)=>{
    surveyPopup(ticketNumber,isQuotationSubmitted,isPoRecieved);
  };
  const foreclosePopupS=(ticketNumber:any)=>{
    foreclosePopup(ticketNumber);
  };
        ...
        <View style={styles.groupChildLayout1}>
          <View style={[styles.groupView, styles.groupParentLayout]}>
              ...
              {item.isHold?<View
                ...
                <Text style={[styles.outOfScope1, styles.unattendedTypo]}>On Hold</Text>
              </View>:null}
            ...
            <View style={[styles.frameParent5, styles.frameParentPosition3]}>
              ...
              <View style={styles.lineParent}>
                  <View style={[item.isPoRecieved?styles.groupChild4c : styles.groupChild3c, styles.groupChildPosition1c]} />
                  <View style={[item.isPoRecieved?styles.groupChild4c : (item.isWorkInProgress?styles.groupChild4c : styles.groupChild3c), styles.groupChildPositionc]} />
                  <Image
                    style={[styles.ellipseIcon, styles.groupChildLayoutO]}
                    resizeMode="cover"
                    source={item.isPoRecieved?require("../assets/ellipse-195.png"):require("../assets/ellipse-1951.png")}
                  />
                  <Image
                    style={[styles.groupChild5c, styles.groupChildLayoutO]}
                    resizeMode="cover"
                    source={item.isPoRecieved?require("../assets/ellipse-195.png") : (item.isQuotationSubmitted?require("../assets/ellipse-195.png"):require("../assets/ellipse-1951.png"))}
                  />
                  <Image
                    style={[styles.groupChild6O, styles.groupChildLayoutO]}
                    resizeMode="cover"
                    source={require("../assets/ellipse-195.png")}
                  />
                  <Text
                    style={[styles.incomplete1, styles.inProgressTypo]}
                  >Ticket Recieved</Text>
              ...
              <Pressable style={styles.mediumLayout} onPress={() =>surveyPopupS(item.ticketNumber,item.isQuotationSubmitted,item.isPoRecieved)}>
                  <Text numberOfLines={1} style={[styles.survey, styles.approveTypo]}>{item.isPoRecieved?'Approve & Release':(item.isQuotationSubmitted?'P.O. Recieved':'Quotation Submitted')}</Text>
              </Pressable>
                       ...
                       {/* <Pressable
                          ...
                          onPress={() => navigation.navigate("ViewTicket2",{ticketId:item.id,})}>
                            <Image
                                ...
                                resizeMode="cover"
                                source={require("../assets/icon_outscope.png")}
                              />
                            <Text style={styles.listItemStyle}>Request Materials</Text>
                        </Pressable> */}
                        <Pressable
                          style={styles.listItemStyleLink}
                          onPress={() => foreclosePopupS(item.ticketNumber)}>
                            <Image
                                style={styles.groupIconPop}
                                resizeMode="cover"
                                source={require("../assets/forecloseicon.jpg")}
                              />
                            <Text style={styles.listItemStyle}>Foreclose</Text>
                        </Pressable>
                        <Pressable
                          style={styles.listItemStyleLink}
                          onPress={() => navigation.navigate("ViewTicket2",{ticketId:item.id,})}>
...
const OutOfScope = () => {
  ...
  const [pageHeight, setPageHeight] = useState(310);
  ...
  const [isPoRecieved, setIsPoRecieved] = useState(false);
 ...
 useEffect(() => {
    if(requestList.length<2)
    {
      ...
      setPageHeight(260+(requestList.length*330));
    }
}, [requestList]);
  ...
  const hideMessage=async () =>{
     ...
     setIsQuotationSubmitted(false);
     setIsPoRecieved(false);
     setShowSurveyPopup(false);
     setShowForeclose(false);
  };
  
  
  const surveyPopup = async (ticketNumber:string,isQuotationSubmitted:boolean,isPoRecieved:boolean) =>{
    setDeleteReason("");
    setDeleteTicketNumber(ticketNumber);
    setIsQuotationSubmitted(isQuotationSubmitted);
    setIsPoRecieved(isPoRecieved);
    setShowSurveyPopup(true);
 };
 const serveyTicket = async () => {
  if(!isQuotationSubmitted){
    var data={
      ticketNumber:deleteTicketNumber,
      poNumber:''
    }
    WebServer.postDataWithToken('FomMobJobTicketHead/markQuotationSubmitted',token,data, serveyTicketCallback);
  }else if(!isPoRecieved){
    var data={
      ticketNumber:deleteTicketNumber,
      poNumber:''
    }
    WebServer.postDataWithToken('FomMobJobTicketHead/markPoReceived',token,data, serveyTicketCallback);
  }else{
...
const serveyTicketCallback = serveyReponse => {
  if(serveyReponse && serveyReponse.message!=undefined){
    if(serveyReponse.message==="Successful."){
      ...
      // setPageHeight(windowHeight);
       ...
       setShowSurveyPopup(false);
       if(!isQuotationSubmitted){
         Alert.alert("Successfully submitted the ticket quotation.");
       }else if(!isPoRecieved){
         Alert.alert("Successfully completed the ticket P.O. Release.");
  ...
  const foreclosePopup = async (ticketNumber:any) =>{
    setDeleteReason("");
    setDeleteTicketNumber(ticketNumber);
    setShowForeclose(true);
  };
  
  const forecloseTicket = async () => {
    var data={
      ticketNumber:deleteTicketNumber,
      reasonCode:deleteReason
    }
    console.log('forecloseTicket : '+JSON.stringify(data));
    WebServer.postDataWithToken('FomMobJobTicketHead/forecloseJobTicket',token,data, forecloseTicketCallback);
  };
  const forecloseTicketCallback = reponse => {
    if(reponse && reponse.message!=undefined){
      if(reponse.message==="Successful."){
       ...
       setShowForeclose(false);
        ...
        Alert.alert("Successfully foreclosed the ticket");
  ...
  const renderData = () => {
    return requestList.map((item:any) => (
      <ListItem key={item.id} item={item} surveyPopup={surveyPopup} deletePopup={deletePopup} foreclosePopup={foreclosePopup}  />
...
const getStatusCountDataCallback = statusCountDataReponse => {
   if(statusCountDataReponse!=null && statusCountDataReponse!=undefined){
    console.log(JSON.stringify(statusCountDataReponse))
...
const getRequestList = async (jwttoken:string) => {
    ...
    WebServer.postDataWithToken('FomMobJobTicketHead/getOutOfScopeTicketsListPagination',jwttoken,params, requestListCallback);
  };
                             ...
                             <Pressable
                              ...
                              >
                                ...
                                <View style={[styles.frameParent3, styles.frameParent3Position]}>
                                  ...
                                  <Text style={[styles.text6, styles.textSpaceBlock]}>({arrivedCount})</Text>
                                </View>
                              </Pressable>
                              ...
                              <View style={styles.rectangleLayout}>
                                  ...
                                  <View style={[styles.frameParent, styles.frameParentPosition4]}>
                                    ...
                                    <Text style={[styles.text, styles.textSpaceBlock]}>({outOfScopeCount})</Text>
                                  </View>
                              </View>
  ...
  <AfterLoginFooter currentRoute='FrontOffice'/>
          ...
          {showSurveyPopup?<View style={{backgroundColor: Color.colorGray_600,
            ...
            {showSurveyPopup?<View style={[styles.rectangleParentS, styles.groupInnerPosition5]}>
              ...
              <Text style={[styles.determineClosureReason, styles.closeTypo]}>
                  {!isQuotationSubmitted?'Quotation Submitted':(!isPoRecieved?'P.O. Recieved':'Approve & Release')}
                </Text>
                <Pressable
                    style={styles.vector}
                    onPress={hideMessage}
              ...
              <View style={[styles.reasonForClosure, styles.reasonForClosureStyle]}>
                ...
                <CheckBox 
                     ...
                     isChecked={isChecked}
                     rightText= {!isQuotationSubmitted?'Quotation Submitted':(!isPoRecieved?'P.O. Recieved':'Approve & Release')}/>
                  {/* <View style={[styles.groupChildp, styles.childGroupShadowBox]} />
                  <TextInput
                        multiline
                        numberOfLines={4}
          ...
          {showForeclose?<View style={{backgroundColor: Color.colorGray_600,
                        width: "100%",
                        position: "absolute",zIndex:1,
                        height: windowHeight}} />:null}
            {showForeclose?<View style={[styles.rectangleParent1, styles.groupInnerPosition]}>
              <View style={[styles.groupInnerp, styles.groupInnerPosition]} />
              <Text style={[styles.determineClosureReason, styles.closeTypo]}>
                  Foreclose
                </Text>
              ...
              <View style={[styles.reasonForClosure, styles.reasonForClosureStyle]}>
                ...
                <View style={[styles.rectangleParent3, styles.groupChildLayoutp]}>
                  ...
                  <Pressable
                      style={[styles.button, styles.buttonPosition]}
                      onPress={forecloseTicket}>
                     <View style={[styles.buttonChild, styles.buttonPosition]} />
                     <Text style={[styles.close, styles.closeTypo]}>Foreclose</Text>
                  </Pressable>
          </View>:null}
