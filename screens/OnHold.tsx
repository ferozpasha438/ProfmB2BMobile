...
const ListItem = ({ item,surveyPopup,deletePopup,foreclosePopup,releaseOnholdPopup }) => {
  ...
  const releaseOnholdPopupS=(ticketNumber:any,isConvertedWorkOrder:boolean)=>{
    releaseOnholdPopup(ticketNumber,isConvertedWorkOrder);
  };
        ...
        <View style={styles.groupChildLayout1}>
          <View style={[styles.groupView, styles.groupParentLayout]}>
            ...
            <View style={[styles.frameParent5, styles.frameParentPosition3]}>
              ...
              <Pressable style={styles.mediumLayout}  onPress={() => releaseOnholdPopupS(item.ticketNumber,item.isConvertedToWorkOrder)}>
                  <Text style={[styles.survey, styles.approveTypo]}>{item.isConvertedToWorkOrder?'Job Completed':'Create Work Order'}  </Text>
              </Pressable>
...
const OnHold = () => {
  ...
  const [releaseOnHold, setReleaseOnHold] = useState(false);
  const [isConvertedWorkOrder, setIsConvertedWorkOrder] = useState(false);
 ...
 useEffect(() => {
  ...
  const hideMessage=async () =>{
     ...
     setReleaseOnHold(false);
     setIsConvertedWorkOrder(false);
  };
...
const serveyTicketCallback = serveyReponse => {
  ...
  const releaseOnholdPopup = async (ticketNumber:any,isConvertedWorkOrder:boolean) =>{
    setDeleteReason("");
    setDeleteTicketNumber(ticketNumber);
    setReleaseOnHold(true);
    setIsConvertedWorkOrder(isConvertedWorkOrder);
  };
  const releaseOnHoldTicket = async () => {
    WebServer.getDataWithToken('FomMobJobTicketHead/changeOnHoldStateJobTicketHeadById?ticketNumber='+deleteTicketNumber,token,'', releaseOnHoldTicketCallback);
  };
  const releaseOnHoldTicketCallback = reponse => {
    if(reponse && reponse.message!=undefined){
      if(reponse.message==="Successful."){
        setPage(0);
        setDeleteReason("");
  ...
  const renderData = () => {
    return requestList.map((item:any) => (
      <ListItem key={item.id} item={item} surveyPopup={surveyPopup} deletePopup={deletePopup} foreclosePopup={foreclosePopup} releaseOnholdPopup={releaseOnholdPopup}  />
...
const getRequestList = async (jwttoken:string) => {
    ...
    WebServer.postDataWithToken('FomMobJobTicketHead/getOnHoldTicketsListPagination',jwttoken,params, requestListCallback);
  };
    ...
    <>
                      ...
                      <ScrollView  horizontal={true}
                              ...
                              >
                                ...
                                <View style={styles.frameParentPosition4}>
                                  ...
                                  <Text style={[styles.text5, styles.textSpaceBlock]}>({approvedCount})</Text>
                                </View>
                              </Pressable>
                              ...
                              <View style={styles.rectangleLayout}>
                                  ...
                                  <View style={[styles.frameParent, styles.frameParentPosition4]}>
                                    ...
                                    <Text style={[styles.text, styles.textSpaceBlock]}>({onHoldCount})</Text>
                                  </View>
                              </View>
     ...
     <View style={styles.groupParentFrontOffice}>
        ...
        <AfterLoginFooter currentRoute='FrontOffice'/>
          ...
          {releaseOnHold?<View style={{backgroundColor: Color.colorGray_600,
                        width: "100%",
                        position: "absolute",zIndex:1,
                        height: windowHeight}} />:null}
          {releaseOnHold?<View style={[styles.rectangleParentS, styles.groupInnerPosition5,{height:380}]}>
              <View style={[styles.groupInnerp, styles.groupInnerPosition5,{height:380}]} />
              {/* <Text style={[styles.determineClosureReason, styles.closeTypo]}>
                    Back To Previous
                </Text> */}
                <Pressable
              ...
              <View style={[styles.reasonForClosure, styles.reasonForClosureStyle]}>
                  <Image source={require("../assets/36fd3a07-b9cf-4d50-83e0-55a080f6c398.gif")} style={styles.httpslottiefilescomanimatIcon}
                         resizeMode="cover"/>
                <View style={[styles.rectangleParent150, styles.groupChildLayoutp,{top:130}]}>
                   <Text style={styles.textBackTo}>Are You Sure You Want To Go Back TO The Previous Step? </Text>
                </View>
              </View>
              <Pressable
                  style={[styles.button5, styles.buttonPosition,{top:300}]}
                  onPress={releaseOnHoldTicket}>
                  <View style={[styles.buttonChildForComplete, styles.buttonPosition]} />
                  <Text style={[styles.close, styles.closeTypo]}>Ticket Resumption</Text>
                </Pressable>
